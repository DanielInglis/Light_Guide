...
//
  ...
  myMPT4->AddConstProperty("FASTTIMECONSTANT", 10.*ns);
  myMPT4->AddConstProperty("SLOWTIMECONSTANT",1000.*ns);
  myMPT4->AddConstProperty("YIELDRATIO",0.8);
...
// The Water Guide
//
  G4Trd* Guide_box = new G4Trd("Guide",25.2*mm,3*mm,25.2*mm,3*mm,5*mm);

  G4LogicalVolume* Guide_log
    = new G4LogicalVolume(Guide_box,perspex,"Guide",0,0,0);

  G4VPhysicalVolume* Guide_phys
    = new G4PVPlacement(0,G4ThreeVector(),Guide_log,"Guide",
                        expHall_log,false,0);

// The Scintillator
//
  G4Tubs* Scint_box = new G4Tubs("Scint",0*mm,25.5*mm,35.75*mm,0.*deg,360.*deg);

  G4LogicalVolume* Scint_log
    = new G4LogicalVolume(Scint_box,CLYC,"Scint",0,0,0);

//G4VPhysicalVolume* Scint_phys =
      new G4PVPlacement(0,G4ThreeVector(0,0,-40.5*mm),Scint_log,"Scint",
                        expHall_log,false,0);
...
//
  ...
  opGuideSurface->SetFinish(ground);
